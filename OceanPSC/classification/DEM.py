...
from .quadratic_parametrisation import quadratic_parametrisation
...
class DEM(object):
    ...
    def from_file(path,slope_path=None,roughness_path=None,organization_path=None,curvature_path=None):
        ...
        dem=DEM(data)
        if slope_path != None:
            dem.slope=np.load(slope_path)
        if roughness_path != None:
            dem.roughness=np.load(roughness_path)
        if organization_path != None:
            dem.organization=np.load(organization_path)
        if curvature_path != None:
            dem.curvature=np.load(curvature_path)
        return dem
    ...
    def classify_area(self,radius):
        ...
        a,b,c,d,e,f=quadratic_parametrisation(self.elevation,radius,self.unit)
        return self.classify(a,b,c,d,e,f,radius)


    def calculate_all_indicators(self,r_range=range(1,16),splits=10,print_progress=True):
        ...
        splits is used to divide_treatment on the map in order to avoid memory error (it should divide the shape) (see operations.divide_treatment for details).
        print_progress if you want to see the percentage at each step (the calculation might take hours).
        """

        #we use an auxiliary function for divide treatment
        def calc(study_area):
            #slope
            ...
            cpt=0
            for r in r_range:
                a,b,c,d,e,f=quadratic_parametrisation(study_area,r,self.unit)
                ...
                #classification
                clasr=self.classify(a,b,c,d,e,f,r)
            ...
            return roughness,avg_slope,organization,avg_curvature

        roughness,avg_slope,organization,avg_curvature=op.divide_treatment(self.elevation,calc,r=splits,result_len=4,display=print_progress)
